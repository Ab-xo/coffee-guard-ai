...
- **OOD Threshold Adjustment** - Increased Out-of-Distribution detection threshold from 0.5536 to 0.68
  - Resolves issue where valid coffee leaf photos were incorrectly rejected
  - Threshold was 99.5th percentile (too strict for real-world use)
  - New threshold (0.68) maintains OOD detection while accepting ~95-97% of real-world coffee leaves
  - Preserves strong rejection of non-coffee images
  - See `docs/decisions/002-ood-threshold-adjustment.md` for full analysis

### Changed

- Updated `artifacts/models/coffeeguard-effv2b0-v1/bundle.json` with new OOD threshold
- Documented original threshold value for reference
